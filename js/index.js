...
addEventListener("DOMContentLoaded", () => {
    muestraCard('information', "Explicacion del juego  <br><br>", `1- El juego es totalmente aleatorio....<br>2- Los intentos seran calculados en base a la dificultad de la ronda.<br>
 3- Para ganar, debes encontrar el numero antes de agotar tus intentos.<br>
   4- Cada vez que falles, se restara un intento y se eliminara el ultimo premio.`, 10);
    iniciaRonda();
...
function ganador() {
    ...
    ++nivel;
    muestraCard('winner', 'Felicidades has ganado', `Nivel: ${nivel} Actual... Niveles superados: ${nivel > 1 ? nivel - 1 : 0
        }
  Puntos Acumulados: ${puntosAcumulados} +${puntosGanar} Puntos`, 3);
}
...
function perdedor() {
    ...
    if (premio.length == 0) {
        ...
        muestraCard('loser', 'Has perdido 😞😞', `Nivel: ${nivel} Actual... Niveles superados: ${nivel > 1 ? nivel - 1 : 0
            }
  Puntos Acumulados: ${puntosAcumulados} +${puntosGanar} Puntos`, 3);
    }
}
...
function muestraCard(nameClass = null, titleCard = "error", infoCard = "error", segundos = 3) {
